...
class SimilarityTrans:
    ...
    def __call__(self, img, lmk):
        ...
        if np.any(lmk < 0) or np.any(np.isnan(M)):
            # in case there are no landmarks detected, affine trans by the identity matrix
            M = np.zeros_like(M)
            M[0, 0] += self.out_size[0]/img.shape[1]
            M[1, 1] += self.out_size[1]/img.shape[0]
