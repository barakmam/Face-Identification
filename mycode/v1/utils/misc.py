...
def filter_videos(embedding_path, state, splits_path='/inputs/bionicEye/data/ytf/splits', head_pose_file=None,
                  num_frames=None, detection_file=None, detection_filters=None, score_filter=-1):
    if state == 'all_data':
        feats, id_, filename = \
            filter_videos_by_videoname(embedding_path,
                                       filter_videoname_path=os.path.join(splits_path, 'all_data.pickle'))
    elif state == 'test_query':
        feats, id_, filename = \
            filter_videos_by_videoname(embedding_path,
                                       filter_videoname_path=os.path.join(splits_path, 'test/query.pickle'))
    elif state == 'test_gallery':
        feats, id_, filename = \
            filter_videos_by_videoname(embedding_path,
                                       filter_videoname_path=os.path.join(splits_path, 'test/gallery.pickle'))
    elif state == 'train':
        feats, id_, filename = \
            filter_videos_by_videoname(embedding_path,
                                       filter_videoname_path=os.path.join(splits_path, 'train/train.pickle'))
    elif state == 'train_query':
        feats, id_, filename = \
            filter_videos_by_videoname(embedding_path,
                                       filter_videoname_path=os.path.join(splits_path, 'train/query.pickle'))
    elif state == 'train_gallery':
        feats, id_, filename = \
            filter_videos_by_videoname(embedding_path,
                                       filter_videoname_path=os.path.join(splits_path, 'train/gallery.pickle'))
    elif state == 'val':
        feats, id_, filename = \
            filter_videos_by_videoname(embedding_path,
                                       filter_videoname_path=os.path.join(splits_path, 'val/val.pickle'))
    elif state == 'val_query':
        feats, id_, filename = \
            filter_videos_by_videoname(embedding_path,
                                       filter_videoname_path=os.path.join(splits_path, 'val/query.pickle'))
    elif state == 'val_gallery':
        feats, id_, filename = \
            filter_videos_by_videoname(embedding_path,
                                       filter_videoname_path=os.path.join(splits_path, 'val/gallery.pickle'))
    else:
    ...
    if score_filter >= 0:
        ...
        filter_inds = get_score_inds(filename, detection['score'], 0)
        if head_pose_file:
            ...
            feats = [feats[ii][frontal_inds[ii]] for ii in range(len(feats))]
            filename = [filename[ii][frontal_inds[ii]] for ii in range(len(filename))]
            return feats, id_, filename, frontal_inds

        feats = [feats[ii][filter_inds[ii]] for ii in range(len(feats))]
        filename = [filename[ii][filter_inds[ii]] for ii in range(len(filename))]
        return feats, id_, filename
...
def filter_templates(embedding_path, state, splits_path='/inputs/bionicEye/ms1m-retinaface-t1/splits', min_num_images=None):
    if state == 'all_data':
        feats, id_, filename = \
            filter_templates_by_ids(embedding_path, filter_ids_path=os.path.join(splits_path, 'all_data.pickle'))
    elif state == 'test_query':
        feats, id_, filename = \
            filter_templates_by_ids(embedding_path, filter_ids_path=os.path.join(splits_path, 'test/query.pickle'))
    elif state == 'test_gallery':
        feats, id_, filename = \
            filter_templates_by_ids(embedding_path, filter_ids_path=os.path.join(splits_path, 'test/gallery.pickle'))
    elif state == 'train':
        feats, id_, filename = \
            filter_templates_by_ids(embedding_path, filter_ids_path=os.path.join(splits_path, 'train/train.pickle'))
    elif state == 'train_query':
        feats, id_, filename = \
            filter_templates_by_ids(embedding_path, filter_ids_path=os.path.join(splits_path, 'train/query.pickle'))
    elif state == 'train_gallery':
        feats, id_, filename = \
            filter_templates_by_ids(embedding_path, filter_ids_path=os.path.join(splits_path, 'train/gallery.pickle'))
    elif state == 'val':
        feats, id_, filename = \
            filter_templates_by_ids(embedding_path, filter_ids_path=os.path.join(splits_path, 'val/val.pickle'))
    elif state == 'val_query':
        feats, id_, filename = \
            filter_templates_by_ids(embedding_path, filter_ids_path=os.path.join(splits_path, 'val/query.pickle'))
    elif state == 'val_gallery':
        feats, id_, filename = \
            filter_templates_by_ids(embedding_path, filter_ids_path=os.path.join(splits_path, 'val/gallery.pickle'))
    else:
        raise NotImplementedError(f'state - {state} - not implemented!')

    filter_num_images_inds = [len(template) >= min_num_images for template in feats]
...
def get_score_inds(filenames, score, thresh=0):
